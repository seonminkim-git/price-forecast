...
from func_model import Prophet, ARIMAX, XGB, TabNet
...
for trsz in train_size:  # 학습 데이터 사이즈. 현 시점 기준 몇 달 전 데이터까지 사용할지
    for tssz in test_size:  # 테스트 데이터 ?!?!. 현 시점 기준 몇 달 후 값을 예측할지
        ...
        for i in test_idx:  # 테스트 기간 한 포인트마다 windowing하며 학습 & 테스트
            ...
            train_idx = list(range(max(0, i-trsz-tssz), i-tssz))
            y_idx = np.array(train_idx) + tssz

            """ split into train/test set """
            x_train, y_train = X.iloc[train_idx], y.iloc[y_idx]
            x_test, y_test = X.iloc[i-tssz], pd.Series(y.iloc[i], index=[y.index[i]])
