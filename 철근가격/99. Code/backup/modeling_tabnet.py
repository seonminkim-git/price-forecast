...
if do_tabnet:
    for algo, ip, fv in feature_set:
        for trival in train_ival:
                ...
                for i in test_idx:
                    ...
                    if fv is None or not any(np.where(fv.iloc[train_idx].isnull())[0]):
                        ...
                        predict_table.to_csv(
                            './' + str(algo) + '_변수_' + str(ip) + '_학습기간_' + str(trival) + '_예측기간_' + str(tsival) + '.csv', index=True, encoding='utf-8-sig')
            ...
            print(result)
            result.to_csv('./final_' + str(algo) + '_학습기간_' + str(trival) + '_예측기간_' + str(
                tsival) + '_performance.csv', index=False, encoding='utf-8-sig')
#                 result.to_csv('./result/final_' + str(algo) +'_.csv', index=False, encoding='utf-8-sig')
